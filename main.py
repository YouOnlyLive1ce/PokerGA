...
class GeneticPlayer(Player):
    ...
    def estimate_bet(self, hand, board):
        ...
        try:
            ...
            self.postflop_ranges[0] = 1
            self.postflop_ranges[5] = 7462
            bet_coefficient = self.pot_coefficients[4]
...
def crossover(parent1, parent2):
    parent1_postflop = parent1.postflop_ranges[:3]
    parent2_postflop = parent2.postflop_ranges[3:]
    child_postflop = parent1_postflop + parent2_postflop
    child = GeneticPlayer()
    child.postflop_ranges = child_postflop
    child.stack = parent1.stack
    return child

def mutate(player):
    mutate_bound = sample(range(1, 7462), 1)[0]
    mutate_index = bisect.bisect_left(player.postflop_ranges, mutate_bound)
    player.postflop_ranges[mutate_index - 1] = mutate_bound
    player.postflop_ranges.sort()
    return player

def store_info(generation_number):
    with open(f'results_{generation_number}.txt', 'w') as file:
        for i in range(len(genetic_players)):
            file.write(
                f"Player #{i}: stack: {genetic_players[i].stack}, "
                f"postflop: {genetic_players[i].postflop_ranges}\n")
...
while True:
    ...
    # Set all the stacks back to 100
    for i in range(len(genetic_players)):
        genetic_players[i].stack = 100
    ...
    print(f"All games played")
    sorted_players = sorted(genetic_players, key=lambda player: player.fitness())

    offspring = []
    for i in range(int(crossover_rate * amount_chromosomes)):
        # Roulette wheel selection
        parent1 = random.choices(sorted_players, weights=[player.fitness() for player in sorted_players])[0]
        parent2 = random.choices(sorted_players, weights=[player.fitness() for player in sorted_players])[0]
        child = crossover(parent1, parent2)
        offspring.append(child)

    num_to_replace = len(offspring)
    sorted_players[:num_to_replace] = offspring
    print(f"Crossover completed")
    for i in range(int(mutate_rate * amount_chromosomes)):
        player_index = random.randint(0, amount_chromosomes - 1)
        mutated_player = mutate(sorted_players[player_index])
        sorted_players[player_index] = mutated_player

    print(f"Mutation completed")

    genetic_players = sorted(sorted_players, key=lambda player: player.fitness())
    generation += 1
    store_info(generation)
